...
class FontClampEnhancedCoreInterface {
   ...
   */
  initializeData() {
    const data = window.fluidfontforgeAjax?.data || {};
  ...
  bindToggleEvents() {
    setTimeout(() => {
      ...
      const sampleTextToggle = document.querySelector(
        '[data-toggle-target="sample-text-content"]'
      );
      const fontPreviewToggle = document.querySelector(
        '[data-toggle-target="font-preview-content"]'
      );
      ...
      if (sampleTextToggle) {
        sampleTextToggle.addEventListener("click", (e) => {
          e.preventDefault();
          this.handleToggle(sampleTextToggle, "sample-text-content");
        });
      }
      if (fontPreviewToggle) {
        fontPreviewToggle.addEventListener("click", (e) => {
          e.preventDefault();
          this.handleToggle(fontPreviewToggle, "font-preview-content");
        });
      }
    }, 1000);
  }
...
class FontClampAdvanced {
  ...
  constructor() {
    ...
    this.version = window.fluidfontforgeAjax?.version || "4.0.5";
  ...
  initializeConstants() {
    return (
      window.fluidfontforgeAjax?.constants || {
...
<div class="fcc-btn-group">
  ...
  saveEdit() {
    ...
    if (this.isAddingNew) {
      ...
      if (window.FontForgeData) {
        ...
        if (activeTab === "class") {
          window.fluidfontforgeAjax.data.classSizes = sizes;
        } else if (activeTab === "vars") {
          window.fluidfontforgeAjax.data.variableSizes = sizes;
        } else if (activeTab === "tag") {
          window.fluidfontforgeAjax.data.tagSizes = sizes;
        } else if (activeTab === "tailwind") {
          window.fluidfontforgeAjax.data.tailwindSizes = sizes;
        }
      }
    }
        ...
        if (window.FontForgeData) {
          ...
          switch (activeTab) {
            case "class":
              ...
              window.fluidfontforgeAjax.data.classSizes = defaultClassSizes;
              break;
            case "vars":
          ...
          if (window.fluidfontforgeAjax.data.explicitlyClearedTabs) {
            delete window.fluidfontforgeAjax.data.explicitlyClearedTabs[
              activeTab
            ];
          }
        }
    ...
    const currentData = window.FontForgeData
      ...
      : [
          ...(TabDataUtils.getDataForTab(
            activeTab,
            window.fluidfontforgeAjax?.data
          ) || []),
        ];
  ...
  clearDataSources(context) {
    if (window.FontForgeData) {
      ...
      if (window.fluidfontforgeAjax?.data) {
        window.fluidfontforgeAjax.data[context.dataArrayRef] = [];
        window.fluidfontforgeAjax.data.explicitlyClearedTabs =
          window.fluidfontforgeAjax.data.explicitlyClearedTabs || {};
        window.fluidfontforgeAjax.data.explicitlyClearedTabs[
          context.activeTab
        ] = true;
...
<button id="undo-clear-btn" class="fcc-btn">undo</button>
    ...
    document.getElementById("undo-clear-btn").addEventListener("click", () => {
      if (window.FontForgeData) {
        ...
        if (window.fluidfontforgeAjax?.data?.explicitlyClearedTabs) {
          delete window.fluidfontforgeAjax.data.explicitlyClearedTabs[tabType];
        }
      } else {
        if (window.fluidfontforgeAjax?.data) {
          window.fluidfontforgeAjax.data[dataArrayRef] = backupData;
          if (window.fluidfontforgeAjax.data.explicitlyClearedTabs) {
            delete window.fluidfontforgeAjax.data.explicitlyClearedTabs[
              tabType
            ];
