...
class CSSGeneratorController {
  ...
  getGenerationContext() {
    ...
    const minScale =
      this.advanced.elements?.minScaleSelect?.value ||
      document.getElementById("min-scale")?.value ||
      "1.125";
    const maxScale =
      this.advanced.elements?.maxScaleSelect?.value ||
      document.getElementById("max-scale")?.value ||
      "1.333";
    const projectCustomer =
      (this.advanced.elements?.projectCustomerInput?.value ||
  ...
  /**
   * Map a scale ratio value to its musical interval name
   *
   * @param {string|number} val - Scale ratio (e.g. 1.125)
  ...
  _scaleName(val) {
    ...
    return map[parseFloat(val)] || String(val);
  }

  /**
   * Build the comment block prepended to all generated CSS output
   *
   * @param {string} descriptor - Tab-specific label (e.g. "FLUID FONT SIZE CLASSES")
   * @param {Object} context - Generation context from getGenerationContext()
   * @param {string} [style="css"] - "css" for /* *\/ blocks, "js" for // lines
   * @returns {string} Formatted header comment
   */
  _buildCSSHeader(descriptor, context, style = "css") {
    const {
      minScale, maxScale, minRootSize, maxRootSize,
      minViewport, maxViewport, unitType, projectCustomer,
    } = context;
    ...
    const unit = unitType || "px";
    const minFont = `${minRootSize}${unit}`;
    const maxFont = `${maxRootSize}${unit}`;
    const customerLine = projectCustomer ? `\n   Project / Customer: ${projectCustomer}` : "";

    if (style === "js") {
      const customerLineJs = projectCustomer ? `\n// Project / Customer: ${projectCustomer}` : "";
      return (
        `// ===========================================================================\n` +
        `// ${descriptor}\n` +
        `// Generated by Fluid Font Forge\n` +
        `// Min Viewport Font Scaling: ${this._scaleName(minScale)}\n` +
        `// Max Viewport Font Scaling: ${this._scaleName(maxScale)}\n` +
        `// Min Font / Viewport: ${minFont} / ${minViewport}px\n` +
        `// Max Font / Viewport: ${maxFont} / ${maxViewport}px\n` +
  ...
  generateCSSByType(sizes, context, type) {
    const generators = {
      class: {
        wrapper: (content) =>
          this._buildCSSHeader("FLUID FONT SIZE CLASSES", context) + content,
        rule: (size, clampValue) =>
          `.${size.className} {\n  font-size: ${clampValue};\n  line-height: ${size.lineHeight};\n}\n\n`,
      },
      vars: {
        wrapper: (content) =>
          this._buildCSSHeader("FLUID FONT SIZE VARIABLES", context) +
          `:root {\n${content}}`,
        rule: (size, clampValue) => `  ${size.variableName}: ${clampValue};\n`,
      },
      tailwind: {
        wrapper: (content) =>
          this._buildCSSHeader("FLUID FONT SIZES - TAILWIND CONFIG", context, "js") +
          `module.exports = {\n  theme: {\n    extend: {\n      fontSize: {\n${content}      }\n    }\n  }\n}`,
      ...
      tag: {
        wrapper: (content) =>
          this._buildCSSHeader("FLUID FONT SIZE TAGS", context) + content,
        rule: (size, clampValue) =>
          `${size.tagName} {\n  font-size: ${clampValue};\n  line-height: ${size.lineHeight};\n}\n\n`,
      },
    };
