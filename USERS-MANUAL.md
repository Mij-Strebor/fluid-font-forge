...
- **Project / Customer Labeling**: Tag any configuration with a client or project name (up to 16 characters). The identifier is embedded in the exported JSON filename and in the header comment of every generated CSS block — making it easy to build a named library of settings files, one per project or client. (Feature suggested by @hughes888.)
...
**Export Settings** - Downloads a labeled, timestamped JSON file containing:
...
**Import Settings** - Uploads a JSON file with validation to restore a previous configuration. The importer verifies the file came from Fluid Font Forge before applying it.
...
### Building a Settings Library with Project / Customer Labeling

The **Project / Customer** field in the Settings panel (up to 16 characters: letters, numbers, spaces, and dashes) lets you identify which project or client a configuration belongs to. Once set, the label flows into two places automatically:

**1. The exported JSON filename**

When you click **Export Settings**, the filename includes your label:

```
fluid-font-forge-settings-acme-corp-2026-05-10.json
fluid-font-forge-settings-green-valley-2026-05-10.json
fluid-font-forge-settings-2026-05-10.json   ← no label set
```

A folder of exports sorted by name groups all files for the same client together and makes it immediately obvious which JSON to import.

**2. The generated CSS header comment**

Every generated CSS block (Classes, Variables, Tags, Tailwind) includes a header comment showing the project/customer so the CSS is self-documenting when pasted into a stylesheet:

```css
/* ==========================================================================
   FLUID FONT SIZE CLASSES
   Generated by Fluid Font Forge
   Min Viewport Font Scaling: Major Second
   Max Viewport Font Scaling: Perfect Fourth
   Min Font / Viewport: 16px / 375px
   Max Font / Viewport: 20px / 1620px
...
```
1. Set Project / Customer to "acme-corp"
2. Configure settings and sizes for that client
3. Export → fluid-font-forge-settings-acme-corp-2026-05-10.json
4. Change Project / Customer to "green-valley"
5. Configure for next client
6. Export → fluid-font-forge-settings-green-valley-2026-05-10.json
7. To restore any client: Import their JSON file
8. Generated CSS always identifies the project in its header
```

This workflow was suggested by community member @hughes888.
